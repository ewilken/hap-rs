...
use crate::{
    ...
    service::{accessory_information::AccessoryInformation, input_source, HapService},
    Result,
};
...
impl HapAccessory for InputSourceInner {
    ...
    fn get_services(&self) -> Vec<&dyn HapAccessoryService> { vec![&self.accessory_information, &self.input_source] }

    fn get_mut_services(&mut self) -> Vec<&mut dyn HapAccessoryService> {
        vec![&mut self.accessory_information, &mut self.input_source]
    }
